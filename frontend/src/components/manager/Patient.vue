<template>
    <div>
      <div class="search" style="padding-bottom: 20px">
        <el-input placeholder="请输入身份证号查询" style="width: 200px" v-model="id"/>
        <el-button type="info" plain style="margin: 10px" @click="load(1)">查询</el-button>
        <el-autocomplete
            v-model="name"
            :fetch-suggestions="querySearchPatient"
  ...
  const querySearchPatient = (queryString, cb) => {
    let results = [];
    request
        .post("/patient/querySearch", {queryString: queryString})
        .then((res) => {
          if (res.code === "200") {
            results = res.data;
