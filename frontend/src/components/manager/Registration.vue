<template>
    <div>
      ...
      <div class="operation" style="padding-bottom: 20px" v-if="user.role === 'ADMIN'">
        <el-button type="danger" plain @click="delBatch">批量删除</el-button>
      </div>
  
      <div class="table">
        <el-table :data="tableData" stripe @selection-change="handleSelectionChange" v-if="user.role === 'ADMIN'">
  ...
  const delBatch = () => {
    ...
    ElMessageBox.confirm("您确定批量删除这些数据吗？", "确认删除",
      ...
      request
          .post("/registration/delete/batch", {ids: ids.value})
          .then((res) => {
            if (res.code === "200") {
